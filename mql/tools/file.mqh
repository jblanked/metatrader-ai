...
string fileList(const string folder, const string key = "", const string ext = "", const bool recursive = false)
{
   const string token      = "flist_" + (string)GetTickCount();
   const string tempRel    = "metatrader-ai\\temp\\";
   const string scriptRel  = tempRel + token + ".ps1";
   const string outRel     = tempRel + token + ".txt";
   const string commonPath = FILE_COMMON_FOLDER;
   if(!::FolderCreate("metatrader-ai", FILE_COMMON) || !::FolderCreate(FILE_TEMP_FOLDER, FILE_COMMON))
   {
      FILE_PRINT_RETURN("Failed to create temporary list folder");
   }

   // Normalize extension to leading dot
   string normExt = ext;
   if(normExt != "" && StringSubstr(normExt, 0, 1) != ".")
      normExt = "." + normExt;

   // Escape quotes for PowerShell strings
   string escFolder = folder;
   StringReplace(escFolder, "'", "''");
   string escKey = key;
   StringReplace(escKey, "'", "''");
   string escExt = normExt;
   StringReplace(escExt, "'", "''");
   string escOut = commonPath + outRel;
   StringReplace(escOut, "'", "''");

   // Script file avoids -Filter wildcard issues
   string script = "# generated by fileList\n";
   script += "$ErrorActionPreference = 'SilentlyContinue'\n";
   script += "$dir = '"       + escFolder + "'\n";
   script += "$key = '"       + escKey    + "'\n";
   script += "$ext = '"       + escExt    + "'\n";
   script += "$out = '"       + escOut    + "'\n";
   script += "$recursive = "  + (recursive ? "1" : "0") + "\n";
   script += "try { if ($recursive) { $items = @(Get-ChildItem -LiteralPath $dir -File -Recurse -ErrorAction SilentlyContinue) } ";
   script += "else { $items = @(Get-ChildItem -LiteralPath $dir -File -ErrorAction SilentlyContinue) } } catch { $items = @() }\n";
   script += "$filtered = @($items | Where-Object { ($key -eq '' -or $_.Name -like \"*$key*\") -and ($ext -eq '' -or $_.Extension -ieq $ext) })\n";
   script += "$filtered | ForEach-Object { $_.FullName } | Out-File -FilePath $out -Encoding ascii -Force\n";
   script += "exit 0\n";

   ::FileDelete(scriptRel, FILE_COMMON);
   int sHandle = ::FileOpen(scriptRel, FILE_WRITE | FILE_COMMON | FILE_TXT | FILE_ANSI);
   if(sHandle == INVALID_HANDLE)
   {
      FILE_PRINT_RETURN("Failed to create list script! error: " + (string)GetLastError());
   }
   ::FileWriteString(sHandle, script);
   ::FileClose(sHandle);

   ::FileDelete(outRel, FILE_COMMON);
   const string cmd = "-NoProfile -ExecutionPolicy Bypass -File \"" + commonPath + scriptRel + "\"";
   const int shellResult = ShellExecuteW(0, "open", "powershell.exe", cmd, NULL, 0);
   if(shellResult <= 32)
   {
      FILE_PRINT_RETURN("Failed to run powershell, error: " + (string)shellResult);
   ...
   {
      Sleep(100);
      if(::FileIsExist(outRel, FILE_COMMON))
      {
         ready = true;
         break;
      }
   ...
   if(!ready)
   {
      FILE_PRINT_RETURN("Timed out waiting for list output file!");
   }

   int handle = ::FileOpen(outRel, FILE_READ | FILE_COMMON | FILE_TXT | FILE_ANSI);
   if(handle == INVALID_HANDLE)
   {
